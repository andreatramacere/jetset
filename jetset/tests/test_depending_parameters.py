...
def test_dep_par_log_values(plot=False):
    from jetset.jet_emitters import EmittersDistribution
    import numpy as np
    ...
    j.make_dependent_par(par='gamma_break', depends_on=['gmax'], par_expr='np.log10(gmax /10)')
    j.parameters.gmax.val = 1E5
    np.testing.assert_allclose(j.parameters.gamma_break.val, np.log10(j.parameters.gmax.val/ 10))
    print(j.parameters.gamma_break.val, j.parameters.gmax.val / 10)
    j.save_model('jet.pkl')
    new_jet = Jet.load_model('jet.pkl')
    ...
    j.make_dependent_par(par='gamma0_log_parab', depends_on=['gmax'], par_expr='gmax -1 ')
